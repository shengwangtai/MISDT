...
import ar.com.tadp.xmleditor.basic.BasicXMLEditor;
...
public class ProjectTreeView extends ViewPart {
	...
	private void makeActions()
	{
				...
				if (selectedObject instanceof TreeObject)
				{
					...
					try
					{
						if (fileType.endsWith("bpmn"))	{
							...
							//IPath path = pathFile.getFullPath();
							getViewSite().getWorkbenchWindow().getActivePage().openEditor(new FileEditorInput(pathFile), BPMN2Editor.EDITOR_ID);
							//getSite().getPage().openEditor(new FileEditorInput(pathFile), BPMN2Editor.EDITOR_ID);
						} 
							...
							IFile pathFile = ResourcesPlugin.getWorkspace().getRoot().getProject(selectProjectName).getFolder(selectionTreeObject.getParent().getName()).getFile(selectionTreeObject.getName());
							//IPath path = pathFile.getFullPath();
							getViewSite().getWorkbenchWindow().getActivePage().openEditor(new FileEditorInput(pathFile), BasicXMLEditor.EDITOR_ID);
						}
		...
		//doubleClickAction.setImageDescriptor(ResourceManager.getPluginImageDescriptor("org.eclipse.bpmn2.modeler.ui", "/icons/bpmn2_editor_16.png"));
		
		this.addCollabDiagramAction = new Action("\u65B0\u5EFA\u534F\u4F5C\u6D41\u7A0B\u56FE")
		{
			public void run()
			{
				ISelection selection = treeViewer.getSelection();
		...
		addCollabDiagramAction.setImageDescriptor(ResourceManager.getPluginImageDescriptor("org.eclipse.bpmn2.modeler.ui", "/icons/16/Collaboration.png"));
	
		this.addConvDiagramAction = new Action("\u65B0\u5EFA\u4F1A\u8BDD\u89C6\u56FE")
		{
			public void run()
			{
				ISelection selection = treeViewer.getSelection();
		...
		addConvDiagramAction.setImageDescriptor(ResourceManager.getPluginImageDescriptor("org.eclipse.bpmn2.modeler.ui", "/icons/16/Conversation.png"));
	}

	
	@Override
	public void createPartControl(Composite parent) {
		parent.setFont(SWTResourceManager.getFont("Î¢ÈíÑÅºÚ", 10, SWT.NORMAL));
		// TODO Auto-generated method stub
